...
import { User, UserShop } from '../models/types';
...
export const createUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existings = await sql<User[]>`SELECT * FROM users WHERE phone = ${data.phone}`;
...
export const assignShopToUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { shop_id } = req.body;
    
    if (!req.user?.is_superadmin) {
      const targetUsers = await sql<User[]>`SELECT * FROM users WHERE id = ${id}`;
      const targetUser = targetUsers[0];
      if (targetUser?.business_id !== req.user?.business_id) return res.status(403).json({ status: 'error', message: 'Forbidden' });
    }

    const created_by = req.user?.id || null;
    const userShops = await sql<UserShop[]>`
      INSERT INTO user_shops ${sql({ user_id: id, shop_id, role_id: req.body.role_id, created_by })}
      RETURNING *
    `;
...
export const updateStaffShops = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    (shop_roles || []).forEach((sr: any) => requestedMap.set(sr.shop_id, sr.role_id));
    const requestedShopIds = Array.from(requestedMap.keys());

    const existing = await sql<UserShop[]>`SELECT * FROM user_shops WHERE user_id = ${id}`;
...
export const updateUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!req.user?.is_superadmin) {
       const existings = await sql<User[]>`SELECT * FROM users WHERE id = ${id}`;
    ...
    if (data.password) updateData.pass_hash = await bcrypt.hash(data.password, 10);
    delete updateData.password;

    const users = await sql<User[]>`
      UPDATE users SET ${sql(updateData as any)}
