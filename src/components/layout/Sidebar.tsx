...
import { LayoutDashboard, ShoppingCart, Package, Wallet, LogOut, Settings, Users, BookOpen, X, FileText, Truck, Moon, Sun, ShieldCheck, Briefcase, ShoppingBag } from 'lucide-react';
import { useEffect, useState } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import Select from 'react-select';
import { selectStyles } from '../../lib/utils';

export function Sidebar({ isOpen, closeMenu }: { isOpen?: boolean, closeMenu?: () => void }) {
  ...
  return (
    <div className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border-light)', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
        ...
        <button onClick={logout} className="nav-link" style={{ width: '100%', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', color: 'var(--text-secondary)' }} onMouseOver={e => e.currentTarget.style.color = 'var(--danger)'} onMouseOut={e => e.currentTarget.style.color = 'var(--text-secondary)'}>
          <LogOut size={18} />
          Logout
        </button>
      </div>
